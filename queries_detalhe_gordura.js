...
function recuperarDetalheGordura(req, res, next) {
    ...
    .catch(function (err) {
        ...
        res.status(400)
                .json({
                    status: 'Warning',
                    data_detalhe_gordura: 'N�o existem detalher da gordura cadastrados ou houve algum problema',
                    message: 'Erro: '+err
                });
    });
}
...
function recuperarDetalheGorduraPorVendedor(req, res, next) {
    ...
    .catch(function (err) {
        ...
        res.status(400)
                .json({
                    status: 'Warning',
                    data_detalhe_gordura: 'N�o existe o detalhes da gordura ou houve algum problema',
                    message: 'Erro: '+err
                });
    });
}
    ...
    db.none(query_insert)
        ...
        var tipoVenda = retornaTabelaParaValidacao('saldo_gordura');   
            tipoVenda.then(resSaldoGordura => {
                ...
                //Monta a estrutura de atualiza��o dos saldos
